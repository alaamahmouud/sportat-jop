...
class Helper
{
    ...
    static function notifyByFirebase($title, $body, $tokens, $data = [])        // paramete 5 =>>>> $type
    {
        ...
        $headers = array(
            'Authorization: key=AAAATDTysFI:APA91bHSCBMKLcd1dxhw1QpQxWOpiI5NXxCieHxAPWhgN6ZdwT5RBX2Y0l-DvFJD9BndZDgpzOXuTm_FRPrjbhOugaL7OFL1IgHsfUxKVhajVGtEEtH5h5fHMYSBLzDFhfDesWmjsg2O',
    ...
    static function sendNotification($model,$to, array $notifierIds, $relation, $title, $body, $data_type = 'admin', $data = []): void
    {
        ...
        if (count($notifierIds)) {
            $notification = $model->morphNotifications()->create([
                'title' => $title,
                'content' => $body ,
                'model_type' => $class ,
                'model_id' => $to->id ,
            ...
            if (Token::whereIn('client_id', $notifierIds)->count()) {
                $tokens = Token::whereIn('client_id', $notifierIds)->pluck('token')->toArray();
